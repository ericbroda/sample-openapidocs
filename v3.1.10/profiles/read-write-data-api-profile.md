# Open Banking Read-Write API Profile  - v3.1.10 <!-- omit in toc -->
   ...
   [Overview](#overview)
      ...
      [Design Principles](#design-principles)
         ...
         [Agnostic to Payment Schemes](#agnostic-to-payment-schemes)
         [Status Codes](#status-codes)
         [Unique Identifiers (Id Fields)](#unique-identifiers-id-fields)
         [Categorisation of Implementation Requirements](#categorisation-of-implementation-requirements)
            [Mandatory](#mandatory)
   ...
   [Basics](#basics)
      ...
      [Resource URI Path Structure](#resource-uri-path-structure)
      [Headers](#headers)
         [Request Headers](#request-headers)
         [Response Headers](#response-headers)
      [HTTP Status Codes](#http-status-codes)
         [400 (Bad Request) v/s 404 (Not Found)](#_400-bad-request-v-s-404-not-found)
         [403 (Forbidden)](#_403-forbidden)
         [401 (Unauthorized)](#_401-unauthorized)
         [429 (Too Many Requests)](#_429-too-many-requests)
      [Pre-Conditions](#pre-conditions)
         [Pre-conditions for TPPs](#pre-conditions-for-tpps)
         [Pre-conditions for ASPSPs](#pre-conditions-for-aspsps)
      [Idempotency](#idempotency-2)
      [Message Signing](#message-signing-2)
         [Overview](#overview-2)
         [Key Stores](#key-stores)
         [Specification](#specification)
         [Process for Signing a Payload](#process-for-signing-a-payload)
            [Step 1: Identify the Private Key and Corresponding Signing Certificate to be Used for Signing](#step-1-identify-the-private-key-and-corresponding-signing-certificate-to-be-used-for-signing)
            [Step 2: Form the JOSE Header](#step-2-form-the-jose-header)
            [Step 3: Compute the JWS](#step-3-compute-the-jws)
            [Step 4: Add the JWS as a HTTP Header](#step-4-add-the-jws-as-a-http-header)
         [Process for Verifying a Signature](#process-for-verifying-a-signature)
            [Step 1: Extract the components from the JWS](#step-1-extract-the-components-from-the-jws)
            [Step 2: Validate the JOSE Header and Certificate](#step-2-validate-the-jose-header-and-certificate)
            [Step 3: Verify the Signature](#step-3-verify-the-signature)
         [Sample JOSE Header](#sample-jose-header)
      [Message Encryption](#message-encryption-2)
         [Overview](#overview-3)
         [Message Signing and Encryption](#message-signing-and-encryption)
         [Key Stores](#key-stores-2)
         [Encrypting Non-JSON Data](#encrypting-non-json-data)
      [Filtering](#filtering)
      [Pagination](#pagination)
      [Archiving](#archiving)
      [Supplementary Data](#supplementary-data)
   [Security & Access Control](#security-access-control)
      [Scopes & Grant Types](#scopes-grant-types)
      [Length of Authorization Code, Access Token and Refresh Token](#length-of-authorization-code-access-token-and-refresh-token)
      [Consent Authorisation](#consent-authorisation)
         [Error Condition](#error-condition)
         [Token Expiry Time](#token-expiry-time)
         [Exemptions from Strong Customer Authentication](#exemptions-from-strong-customer-authentication)
      [Supported Grant Types](#supported-grant-types)
         [Grant Types for identifying the TPP](#grant-types-for-identifying-the-tpp)
            [Client Credentials Grant](#client-credentials-grant)
         [Grant Types for identifying the TPP and PSU](#grant-types-for-identifying-the-tpp-and-psu)
            [Authorization Code Grant & Hybrid Grant](#authorization-code-grant-hybrid-grant)
            [id_token_hint](#id-token-hint)
            [jwt_bearer](#jwt-bearer)
         [CIBA](#ciba)
            [Identifying the PSU](#identifying-the-psu)
               [Identifying the consent to be authorised](#identifying-the-consent-to-be-authorised)
               [Identifying the PSU Using a User Id](#identifying-the-psu-using-a-user-id)
               [Identifying the PSU Using an Ephemeral User Id](#identifying-the-psu-using-an-ephemeral-user-id)
               [Identifying the PSU Using an Intent Id](#identifying-the-psu-using-an-intent-id)
               [Identifying the PSU Using a Previously Issued Id Token](#identifying-the-psu-using-a-previously-issued-id-token)
      [Changes to an Intent's Authorized State](#changes-to-an-intent-s-authorized-state)
         [Effect of Token Expiry on an Intent's Authorized State](#effect-of-token-expiry-on-an-intent-s-authorized-state)
      [Consent Re-authentication](#consent-re-authentication)
         [Ability to re-authenticate an Authorised consent at any point of time](#ability-to-re-authenticate-an-authorised-consent-at-any-point-of-time)
         [Low Friction User Experience](#low-friction-user-experience)
         [Use of Refresh Token](#use-of-refresh-token)
         [Consent re-authentication through TPP](#consent-re-authentication-through-tpp)
   ...
   [Usage Examples](#usage-examples)
      [Pagination Flows](#pagination-flows)
         [Request](#request)
         [Paginated Resource Response](#paginated-resource-response)
         [Request Next Page of Results](#request-next-page-of-results)
         [Paginated Resource Response](#paginated-resource-response-2)
      [Error Flows](#error-flows)
         [Missing or Expired Access Token](#missing-or-expired-access-token)
         [Incomplete or Malformed Request Payload](#incomplete-or-malformed-request-payload)
         [Missing or Invalid Access Token Scope](#missing-or-invalid-access-token-scope)
         [Sudden Burst of API Requests](#sudden-burst-of-api-requests)
         [Failed Authorisation Consent](#failed-authorisation-consent)
